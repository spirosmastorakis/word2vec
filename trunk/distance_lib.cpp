...
int get_random_words_disjoint(char* filename, int num_words, char random_words[num_words][100], bool upperHalf) {
  FILE *f;
  char st1[max_size];
  char *bestw[N];
  char file_name[max_size], st[100][max_size];
  ...
  for (int i = 0; i < num_words; i++) {
    if (upperHalf)
      random_num = (rand() % (words / 2)) + (words / 2);
    else
      random_num = (rand() % (words / 2));
    strcpy(random_words[i], &vocab[random_num * max_w]);
  }
  return 0;
}

int distance(char* filename, char* word, void* results_void) {
  FILE *f;
  char st1[max_size];
  char *bestw[N];
  char file_name[max_size], st[100][max_size];
...
int distance_no_open(void* initStruct_void, char* word, void* results_void,
  ...
  //while (1) {
    ...
    for (a = 0; a < results->nResultsToReturn; a++) {
      ...
      results->similarity[a] = bestd[num_vec[a]];
    }
    // //strcpy(result, bestw[0]);
...
// {
//   char result[100];
//   query_distance(argv[1], argv[2], result);
//   printf("Best match: %s\n", result);
//   return 0;
// }
